...
const memberSchema = new Schema({
  ...
  // Who shepherds this member. A shepherd is a MEMBER of this chapter who sits
  // on the check-up team — not a portal account. shepherdStaffId predates that
  // and pointed at a StaffUser; it is kept so nothing already stored is lost,
  // but nothing writes it any more.
  shepherdMemberId: { type: String, default: '', index: true },
  shepherdStaffId: { type: String, default: '' },  // legacy: assigned shepherd (StaffUser id)
  shepherdName: { type: String, default: '' },     // denormalized snapshot for display
  // The check-up team: the members Shepherding sends out to visit and follow
  // up. They are the pool every shepherd is drawn from, so being on the team
  // is what makes someone assignable. A few of them are alumni, which is why
  // nothing here filters on membershipStage.
  onShepherdTeam: { type: Boolean, default: false, index: true },
  shepherdTeamSince: { type: Date, default: null },
