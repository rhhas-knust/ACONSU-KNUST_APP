...
// display directly, never a provider-shaped object. `chapterId` is stamped on
// every log line only — it never changes who gets texted, that's entirely
// decided by resolveAudience() before this is called.
async function sendBatch(recipients, message, sourceId, chapterId) {
  ...
  if (!isConfigured()) {
    ...
    await Promise.all(numbers.map((n) => logSms(n, message, 'skipped', 'SMS is not configured on the server', sourceId, chapterId)));
    return {
